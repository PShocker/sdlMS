    ...
    ski->call_back = [](entt::entity ent, int action_frame, int action_time)
    {
        ...
        if (owner_mv->foo)
        {
            for (auto e : World::registry->view<Summon>())
            {
                ...
                if (sum->state != Summon::State::DIE && sum->owner == ent && sum->id == u"2221005")
                {
                    sum->destory = Window::dt_now + duration;
                    buff->buffs.at(u"2221005").destory = sum->destory;
                    return;
                }
            }
            Skill *ski = World::registry->try_get<Skill>(ent);
            auto e = load_summon(ski->skiw->node->get_child(u"summon"), u"2221005", ent);
            ...
            buff->buffs.emplace(u"2221005", info);
        }
    };
    SkillWarp::cooldowns[u"2221005"] = Window::dt_now + 1500;
